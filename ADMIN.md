...
2. 检查标题、摘要、发布日期、分类、标签和保存目录。
   - 保存到已有目录：在左侧下拉框选择目录。
   - 保存到新目录：在右侧输入框填写目录名，例如 `notes` 或 `notes/ai`；保存时会自动创建目录。
3. 选择操作：
...
- 新建保存目录只会影响文件存放位置；如果希望它成为新的分类页，还需要在 `src/utils/categories.ts` 的分类映射中新增分类。
- 「学习笔记」分类已配置为 `notes`：分类填 `notes`，保存目录可填 `notes`。
